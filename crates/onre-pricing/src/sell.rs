...
use crate::{
    calculate_fee, calculate_step_price_at, PriceVector, PricingError, MAX_BASIS_POINTS,
    MAX_TOKEN_DECIMALS, PRICE_DECIMALS,
};
use core::cmp::max;
...
pub fn calculate_token_out_amount(
    ...
    let tvl = calculate_tvl(price, liquidity.circulating_supply)?;

    let liquidity_cap = calculate_liquidity_cap(
        liquidity.liquidity,
        tvl,
        liquidity.liquidity_cap_target_bps,
        in_decimals,
        out_decimals,
    )?;
...
fn preview_effective_sell_volume(
    ...
    if time < dampening.epoch_start
        || dampening.epoch_start == 0
        || elapsed >= epoch_duration.saturating_mul(2)
    {
...
fn calculate_tvl(price: u64, supply: u64) -> Result<u64, PricingError> {
    (supply as u128 * price as u128 / 10u128.pow(PRICE_DECIMALS as u32))
        .try_into()
        .map_err(|_| PricingError::MathOverflow)
}

fn calculate_liquidity_cap(
    liquidity: u64,
    tvl: u64,
    target_bps: u16,
    onyc_decimals: u8,
    ...
    let target_in_onyc_scale = tvl as u128 * target_bps as u128 / MAX_BASIS_POINTS;

    let target = target_in_onyc_scale
    ...
    let target: u64 = target.try_into().map_err(|_| PricingError::MathOverflow)?;

    Ok(target.min(liquidity))
}
...
mod tests {
    ...
    fn calculate_amount_out_dampening_end_to_end() {
        ...
        let liquidity = LiquidityParams {
            liquidity: 1_000_000_000,
            liquidity_cap_target_bps: 10_000,
            circulating_supply: 198_000_000_000, // produces liquidity_cap = 2 × raw_out at test price
            dampening: DampeningState {
    ...
    #[test]
    fn unit_price_tvl() {
        let tvl = calculate_tvl(1_000_000_000, 100_000_000_000).unwrap();
        assert_eq!(tvl, 100_000_000_000);
    }

    #[test]
    fn zero_supply_tvl() {
        assert_eq!(calculate_tvl(1_000_000_000, 0).unwrap(), 0);
    }

    // ── calculate_liquidity_cap ────────────────────────────────────────

    #[test]
    fn zero_target_bps_returns_liquidity() {
        assert_eq!(
            calculate_liquidity_cap(500, 1_000_000, 0, 9, 6).unwrap(),
            500
        );
    }

    #[test]
    fn cap_clamped_to_liquidity() {
        // target would be 1_000
        // liquidity = 100 < 1_000 -> 100
        assert_eq!(
            calculate_liquidity_cap(100, 1_000_000, 10_000, 9, 6).unwrap(),
            100
        );
    }

    #[test]
    fn cap_below_liquidity() {
        // tvl=1_000_000, target_bps=5_000 (50%) → target_in_onyc_scale = 500_000
        // target = 500_000 * 10^6 / 10^9 = 500
        // liquidity=1_000 → min(500, 1_000) = 500
        assert_eq!(
            calculate_liquidity_cap(1_000, 1_000_000, 5_000, 9, 6).unwrap(),
            500
        );
    }

    #[test]
    fn zero_target_after_rounding_errors() {
        // tvl=1, target_bps=1 → target_in_onyc_scale = 1*1/10_000 = 0
        // target = 0 → InvalidAmount
        assert_eq!(
            calculate_liquidity_cap(1_000, 1, 1, 9, 6),
